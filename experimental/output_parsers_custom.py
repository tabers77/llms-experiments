...
class JsonOutputFunctionsParser(BaseCumulativeTransformOutputParser[Any]):
    # def __init__(self, json_parser_keys, **kwargs: Any):
    #     self.json_parser_keys = json_parser_keys
    #
        ...
        try:
            if partial:
                ...
                if self.args_only:
                    ...
                    fixed_args_format = fix_json_string(function_call["arguments"])

                    try:
                        return json.loads(
                            fixed_args_format, strict=self.strict
                        )

                    except (json.JSONDecodeError, TypeError) as exc:
                        raise OutputParserException(
                            f'Could not parse function call data: {exc}. Current arguments: {function_call["arguments"]}'
                        )
                else:
                    try:
                        return {
